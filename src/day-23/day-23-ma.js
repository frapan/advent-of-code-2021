...
const parseBeginning = (sequence) =>
  Array.from({ length: 4 }, (_, index) =>
    Array.from(
      { length: sequence.length >> 2 },
      (__, roomIdx) => sequence[(roomIdx << 2) + index]
    )
  )
...
function getMinCost(sequence) {
  ...
  finalRoomConfig =
    'ABCD'.replace(/./g, (letter) => (letter + ',').repeat(roomDepth)) +
    ',,,,,,'
...
function getMoveCost(amphipod, start, end) {
  const startHallPos =
    start.room === 4
      ? validHallwayPositions[start.slot]
      : roomPositions[start.room]
  const endHallPos =
    end.room === 4 ? validHallwayPositions[end.slot] : roomPositions[end.room]
  const hallDist = Math.abs(startHallPos - endHallPos)
  const roomDist =
    (start.room === 4 ? 0 : start.slot + 1) +
    (end.room === 4 ? 0 : end.slot + 1)
  ...
  for (let room = 0; room < rooms.length; room++) {
    ...
    if (room === 4) {
      ...
      for (let slot = 0; slot < slots.length; slot++) {
        ...
        const isRoomAvailable = rooms[ownRoom]
          .slice(freeRoomIndex + 1)
          .every((occupant) => occupant === amphipod)
        const canMoveInRoom =
          isRoomAvailable &&
          validHallwayPositions.every((_, idx) => {
            return (
              idx <= Math.min(slot, ownRoom + 1) ||
              idx >= Math.max(slot, ownRoom + 2) ||
              rooms[4][idx] === null
            )
          })
        if (canMoveInRoom) {
          applyMove(
            { rooms, cost },
            { room, slot },
            { room: ownRoom, slot: freeRoomIndex }
          )
        }
      }
    } else {
      ...
      for (
        let index = room + 2;
        index < validHallwayPositions.length;
        index++
      ) {
        if (rooms[4][index]) {
          break
...
console.log(
  getMinCost(rawSequence.slice(0, 4) + 'DCBADBAC' + rawSequence.slice(4))
)
