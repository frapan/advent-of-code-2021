const useTestInput = false
let input
if (useTestInput) {
  input = [
    '[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]',
    '[[[5,[2,8]],4],[5,[[9,9],0]]]',
    '[6,[[[6,2],[5,6]],[[7,6],[4,7]]]]',
    '[[[6,[0,7]],[0,9]],[4,[9,[9,0]]]]',
    '[[[7,[6,4]],[3,[1,3]]],[[[5,5],1],9]]',
    '[[6,[[7,3],[3,2]]],[[[3,8],[5,7]],4]]',
    '[[[[5,4],[7,7]],8],[[8,3],8]]',
    '[[9,3],[[9,9],[6,[4,9]]]]',
    '[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]',
    '[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]',
  ]
} else {
  input = [
    '[5,[7,[8,4]]]',
    '[[[4,1],[6,[9,3]]],[[7,4],[5,[7,0]]]]',
    '[[6,2],[[[8,6],[5,5]],0]]',
    '[[[5,9],[3,[4,2]]],[[[1,2],0],2]]',
    '[[[[4,3],2],0],[[[1,7],[1,2]],[[8,2],[6,7]]]]',
    '[[[[0,1],9],3],[[4,7],[7,8]]]',
    '[[[[8,7],4],[5,[9,2]]],[[8,[9,6]],[1,8]]]',
    '[[[2,3],[[9,9],[7,0]]],[6,7]]',
    '[8,[[9,9],[8,6]]]',
    '[[[[5,7],[7,1]],[3,[7,6]]],[2,[[5,5],[8,3]]]]',
    '[[[7,0],2],[[[2,2],7],[6,[2,9]]]]',
    '[[6,2],[[0,8],8]]',
    '[[[[2,9],4],9],[1,[[6,9],[7,5]]]]',
    '[[[9,3],[[5,7],[3,1]]],[5,[6,[7,8]]]]',
    '[0,[[8,9],1]]',
    '[[4,[[4,3],4]],[7,[[4,0],0]]]',
    '[[0,[[1,9],[6,1]]],[[[7,0],[5,2]],[[3,8],[0,4]]]]',
    '[[[2,7],[7,[1,6]]],[6,[[8,7],[8,5]]]]',
    '[[9,5],[[1,[2,5]],[8,[2,0]]]]',
    '[6,[[8,[9,4]],[9,8]]]',
    '[[[[2,0],[4,6]],3],[[8,0],4]]',
    '[[[8,8],[[5,7],[5,6]]],5]',
    '[[5,[[7,9],9]],[1,6]]',
    '[[[[5,2],[4,9]],[[1,9],[2,9]]],[[[6,8],[7,5]],[[0,2],4]]]',
    '[1,[5,[[5,5],[1,2]]]]',
    '[[[1,4],[[0,3],7]],[[[9,1],9],[[2,3],7]]]',
    '[[[[6,4],[4,0]],[[3,4],[7,0]]],[[8,7],[5,[0,6]]]]',
    '[[3,[8,[2,8]]],[9,[0,[5,2]]]]',
    '[[7,[[1,8],1]],[6,[6,6]]]',
    '[[[3,[9,4]],[[3,2],[5,2]]],8]',
    '[3,[[4,[4,3]],[5,[9,2]]]]',
    '[[[1,8],[2,[7,5]]],[[0,[8,1]],[2,0]]]',
    '[1,3]',
    '[7,[[[9,6],[8,4]],9]]',
    '[6,4]',
    '[[[8,9],[[3,7],2]],[4,[[5,0],8]]]',
    '[[[[1,8],[7,9]],0],[[[4,4],3],[4,[1,7]]]]',
    '[[[[2,2],[0,9]],[1,2]],[[[9,1],[0,0]],[[1,6],4]]]',
    '[[[[8,1],6],[[3,3],[6,7]]],[[2,3],5]]',
    '[[[[9,0],7],6],[[[3,6],[6,7]],3]]',
    '[[[[1,0],6],[5,[0,0]]],[[[9,7],7],5]]',
    '[[[[5,1],4],[[7,7],[6,2]]],[[0,[6,0]],2]]',
    '[[[[8,3],[0,4]],[[9,9],[3,7]]],[[[2,7],[2,9]],[[2,0],[4,7]]]]',
    '[6,[[[4,8],0],8]]',
    '[[[6,[5,9]],[[0,3],9]],[[[2,5],[9,5]],0]]',
    '[[1,4],[6,[0,[6,2]]]]',
    '[9,[[[3,7],1],7]]',
    '[[[2,3],[[1,2],1]],[[[2,6],[0,1]],[0,[4,1]]]]',
    '[[[0,1],[[0,3],[7,3]]],[[8,7],3]]',
    '[[0,[[1,5],[5,3]]],4]',
    '[[[5,3],[[5,8],6]],[[[6,0],3],[4,1]]]',
    '[8,3]',
    '[[[[5,5],[3,0]],6],[[7,5],[2,[9,4]]]]',
    '[[[3,[3,3]],[[4,7],4]],[[2,0],1]]',
    '[[[0,[2,8]],[4,[7,9]]],[[[5,4],2],2]]',
    '[[3,[7,[1,8]]],[5,[[8,2],0]]]',
    '[[1,9],[[6,[5,9]],8]]',
    '[[5,[5,2]],5]',
    '[[[1,1],[4,3]],1]',
    '[[[[6,9],[4,1]],0],[[[3,0],6],7]]',
    '[[9,[[7,3],6]],[[[7,2],0],[9,9]]]',
    '[[5,4],[[[6,0],[5,1]],7]]',
    '[[[4,0],0],[[[2,6],[4,4]],[[6,8],2]]]',
    '[[[9,6],8],[[0,[9,5]],9]]',
    '[[6,[2,5]],[[[1,8],[9,0]],[[4,0],[5,7]]]]',
    '[5,[[8,[9,9]],[5,[6,8]]]]',
    '[[[7,[9,0]],5],6]',
    '[[9,[[3,7],[3,0]]],[[[7,2],[5,7]],[[0,5],[7,4]]]]',
    '[[7,3],[[6,5],[9,4]]]',
    '[[4,[4,3]],[9,[[2,6],0]]]',
    '[[[6,[0,1]],9],[[7,[3,2]],[[0,1],[5,2]]]]',
    '[[5,[0,[3,1]]],[[[1,1],[8,9]],[[6,3],[0,9]]]]',
    '[[[[2,8],0],[[8,7],4]],[[[9,6],3],[[7,8],[2,3]]]]',
    '[[[[1,0],1],4],[4,9]]',
    '[[[7,8],5],[[[3,7],[5,7]],6]]',
    '[[[8,[7,4]],[[1,6],[6,7]]],[2,4]]',
    '[[7,8],3]',
    '[[0,[4,[3,8]]],[[[1,0],1],6]]',
    '[[[[6,3],7],2],[[4,5],6]]',
    '[[[5,9],[[1,8],1]],[[[1,8],8],[[6,4],0]]]',
    '[[3,[8,[2,8]]],[[[2,8],[4,4]],9]]',
    '[7,[5,[[3,3],3]]]',
    '[3,[1,[0,[3,0]]]]',
    '[[[1,2],4],[9,[[7,1],[5,4]]]]',
    '[[[5,8],[7,[0,7]]],[0,[[2,9],8]]]',
    '[[[7,[2,0]],[1,[4,3]]],[0,[[1,1],[2,0]]]]',
    '[[[2,[2,5]],[4,1]],[0,[6,0]]]',
    '[[[8,3],9],[[[4,3],[5,8]],[[7,0],9]]]',
    '[2,[1,4]]',
    '[[[3,[2,6]],6],[[[3,2],[0,8]],[[3,5],[6,4]]]]',
    '[[[1,[3,3]],[[0,8],[1,3]]],[8,[[3,8],[0,8]]]]',
    '[[[[1,5],[0,1]],3],[[6,[1,7]],[4,7]]]',
    '[[4,[5,7]],[6,[[6,2],7]]]',
    '[[[[7,4],[3,1]],[5,6]],[0,[6,5]]]',
    '[[[7,[0,0]],6],[5,[[0,0],[3,5]]]]',
    '[[[[8,7],[5,8]],[8,[9,3]]],[[7,0],[[7,2],0]]]',
    '[[[7,[4,2]],0],[[[4,0],1],3]]',
    '[[[6,3],[9,[2,2]]],[[0,8],[1,2]]]',
    '[3,[[3,1],[[7,1],1]]]',
    '[[3,[[4,0],7]],[[[4,6],[2,3]],[[0,2],[1,8]]]]',
  ]
}

const explodeValueToLeft = (str, value) => {
  const match = str.match(/\d+/g)
  if (!match) {
    return str
  }
  const oldValue = match[match.length - 1]
  const lastOldValueIndex = str.lastIndexOf(oldValue)
  return (
    str.slice(0, lastOldValueIndex) +
    (Number(oldValue) + value) +
    str.slice(lastOldValueIndex + oldValue.length)
  )
}

const explodeValueToRight = (str, value) => {
  const oldValue = str.match(/\d+/)
  if (!oldValue) {
    return str
  }
  return str.replace(oldValue, Number(oldValue) + value)
}

const explode = (str) => {
  // console.log('explode', str)
  let openedBrackets = 0
  for (let i = 0; i < str.length; i++) {
    if (str[i] === '[') {
      openedBrackets++
    }
    if (str[i] === ']') {
      openedBrackets--
    }
    if (openedBrackets === 5) {
      const commaIdx = str.indexOf(',', i)
      const explodingLeftValue = Number(str.slice(i + 1, commaIdx))
      const closeBracketIdx = str.indexOf(']', commaIdx)
      const explodingRightValue = Number(
        str.slice(commaIdx + 1, closeBracketIdx)
      )
      // console.log({ explodingLeftValue, explodingRightValue })
      const leftStr = explodeValueToLeft(str.slice(0, i), explodingLeftValue)
      const rightStr = explodeValueToRight(
        str.slice(closeBracketIdx + 1),
        explodingRightValue
      )
      return leftStr + '0' + rightStr
    }
  }
  return str
}

const split = (str) => {
  // console.log('split', str)
  const match = str.match(/\d{2,}/)
  if (match) {
    const val = Number(match[0])
    const first = Math.floor(val / 2)
    const last = val - first
    str = str.replace(match[0], `[${first},${last}]`)
  }
  return str
}

const reduce = (str) => {
  let lastStr = str
  while (true) {
    // console.log('reduce', str)
    str = explode(str)
    if (str === lastStr) {
      str = split(str)
      if (str === lastStr) {
        return str
      }
    }
    lastStr = str
  }
}

const add = (str1, str2) => `[${str1},${str2}]`

const getMagnitude = (str) => {
  while (str.indexOf('[') >= 0) {
    str = str.replaceAll(/\[\d+,\d+\]/g, (match) => {
      const [left, right] = match.slice(1, match.length - 1).split(',')
      // console.log(
      //   'match',
      //   match,
      //   left,
      //   right,
      //   Number(left) * 3 + Number(right) * 2
      // )
      return String(Number(left) * 3 + Number(right) * 2)
    })
  }
  return str
}

let highestMagnitude = 0
for (let i = 0; i < input.length; i++) {
  for (let j = 0; j < input.length; j++) {
    if (i !== j) {
      highestMagnitude = Math.max(
        highestMagnitude,
        getMagnitude(reduce(add(input[i], input[j])))
      )
    }
  }
}
// console.log(result)
console.log(highestMagnitude)

// console.log(explode('[[3,[2,[8,0]]],[9,[5,[4,[3,2]]]]]'))
// console.log(reduce('[[[[[4,3],4],4],[7,[[8,4],9]]],[1,1]]'))
// console.log(
//   getMagnitude('[[[[8,7],[7,7]],[[8,6],[7,7]]],[[[0,7],[6,6]],[8,7]]]')
// )
