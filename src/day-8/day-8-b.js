...
const getSignalByLength = (signals, length) =>
  signals.find((s) => s.length === length)
const getSignalsByLength = (signals, length) =>
  signals.filter((s) => s.length === length)
const getSignalCharNotInChars = (signal, chars) =>
  signal.split('').find((char) => !chars.split('').includes(char))
const hasAllChars = (signal, chars) =>
  signal.split('').filter((char) => !chars.split('').includes(char)).length ===
  0

const sortChars = (chars) => {
  const splitted = chars.split('')
  splitted.sort()
  return splitted.join('')
}

const getSegmentMapFromSignals = (signals) => {
  const one = getSignalByLength(signals, 2)
  const seven = getSignalByLength(signals, 3)
  const four = getSignalByLength(signals, 4)
  const eight = getSignalByLength(signals, 7)
  const a = getSignalCharNotInChars(seven, one)
  const sixSegments = getSignalsByLength(signals, 6)
  const nine = sixSegments.find(
    (sixSegment) =>
      hasAllChars(four + a, sixSegment) &&
      getSignalCharNotInChars(sixSegment, four + a)
  )
  const e = getSignalCharNotInChars(eight, nine)
  sixSegments.splice(sixSegments.indexOf(nine), 1)
  const zero = sixSegments.find((sixSegment) => hasAllChars(one, sixSegment))
  sixSegments.splice(sixSegments.indexOf(zero), 1)
  const six = sixSegments[0]
  const fiveSegments = getSignalsByLength(signals, 5)
  const five = fiveSegments.find(
    (fiveSegment) =>
      hasAllChars(fiveSegment, six) &&
      getSignalCharNotInChars(six, fiveSegment) === e
  )
  fiveSegments.splice(fiveSegments.indexOf(five), 1)
  const three = fiveSegments.find((fiveSegment) =>
    hasAllChars(fiveSegment, nine)
  )
  fiveSegments.splice(fiveSegments.indexOf(three), 1)
  const two = fiveSegments[0]
  return {
    [sortChars(zero)]: 0,
    [sortChars(one)]: 1,
    [sortChars(two)]: 2,
...
const getValueByMap = (outputValues, map) => {
  const valueStr = outputValues.reduce((acc, outputValue) => {
    const sortedValue = sortChars(outputValue)
    return `${acc}${map[sortedValue]}`
  }, '')
  return parseInt(valueStr, 10)
}

const result = input.reduce((acc, row) => {
  const [signals, outputValues] = row.split(' | ')
  const map = getSegmentMapFromSignals(signals.split(' '))

  const v = getValueByMap(outputValues.split(' '), map)
  return acc + v
}, 0)
